...
protected:
  ...
  // T-tables: merge SubBytes + ShiftRows + MixColumns into 4x1KB lookups
  alignas(64) static const uint32_t Te0[256]; // encryption round
  alignas(64) static const uint32_t Te1[256];
  alignas(64) static const uint32_t Te2[256];
  alignas(64) static const uint32_t Te3[256];
  alignas(64) static const uint32_t Td0[256]; // decryption round
  alignas(64) static const uint32_t Td1[256];
  alignas(64) static const uint32_t Td2[256];
  alignas(64) static const uint32_t Td3[256];
...
private:
  ...
  void cipherWords(uint32_t s0, uint32_t s1, uint32_t s2, uint32_t s3,
    ...
    s2 ^= rk[2];
    s3 ^= rk[3];
    // Main rounds: SubBytes + ShiftRows + MixColumns + AddRoundKey via T-tables
...
#undef WAES_GEN_ENCRYPT_ROUND
    ...
    store32LE(output,
              (static_cast<uint32_t>(m_sBox[s0 & 0xff]) |
               (static_cast<uint32_t>(m_sBox[(s1 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_sBox[(s2 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_sBox[s3 >> 24]) << 24)) ^
                  rk[0]);
    store32LE(output + 4,
              (static_cast<uint32_t>(m_sBox[s1 & 0xff]) |
               (static_cast<uint32_t>(m_sBox[(s2 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_sBox[(s3 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_sBox[s0 >> 24]) << 24)) ^
                  rk[1]);
    store32LE(output + 8,
              (static_cast<uint32_t>(m_sBox[s2 & 0xff]) |
               (static_cast<uint32_t>(m_sBox[(s3 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_sBox[(s0 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_sBox[s1 >> 24]) << 24)) ^
                  rk[2]);
    store32LE(output + 12,
              (static_cast<uint32_t>(m_sBox[s3 & 0xff]) |
               (static_cast<uint32_t>(m_sBox[(s0 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_sBox[(s1 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_sBox[s2 >> 24]) << 24)) ^
                  rk[3]);
  }
  ...
  void invCipher(const uint8_t *input, uint8_t *output) const {
    ...
    // Initial round key addition (dk[0..3] = encrypt round Nr key)
    ...
    // Main rounds: InvSubBytes + InvShiftRows + InvMixColumns + AddRoundKey via
    // Td-tables
    // InvShiftRows is folded into the table selection below.
...
#undef WAES_GEN_DECRYPT_ROUND
    ...
    store32LE(output,
              (static_cast<uint32_t>(m_invSbox[s0 & 0xff]) |
               (static_cast<uint32_t>(m_invSbox[(s3 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_invSbox[(s2 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_invSbox[s1 >> 24]) << 24)) ^
                  dk[0]);
    store32LE(output + 4,
              (static_cast<uint32_t>(m_invSbox[s1 & 0xff]) |
               (static_cast<uint32_t>(m_invSbox[(s0 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_invSbox[(s3 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_invSbox[s2 >> 24]) << 24)) ^
                  dk[1]);
    store32LE(output + 8,
              (static_cast<uint32_t>(m_invSbox[s2 & 0xff]) |
               (static_cast<uint32_t>(m_invSbox[(s1 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_invSbox[(s0 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_invSbox[s3 >> 24]) << 24)) ^
                  dk[2]);
    store32LE(output + 12,
              (static_cast<uint32_t>(m_invSbox[s3 & 0xff]) |
               (static_cast<uint32_t>(m_invSbox[(s2 >> 8) & 0xff]) << 8) |
               (static_cast<uint32_t>(m_invSbox[(s1 >> 16) & 0xff]) << 16) |
               (static_cast<uint32_t>(m_invSbox[s0 >> 24]) << 24)) ^
                  dk[3]);
  }

  // Apply InvMixColumns to one 32-bit state column (used only during key
  // expansion)
  static uint32_t invMixCol(uint32_t col) noexcept {
    ...
    return Td0[m_sBox[b0]] ^ Td3[m_sBox[b1]] ^ Td2[m_sBox[b2]] ^
           Td1[m_sBox[b3]];
  }
  ...
  void keyExpansion(const uint8_t *key) {
    ...
    auto subWord = [](uint32_t word) {
      return static_cast<uint32_t>(m_sBox[word & 0xff]) |
             (static_cast<uint32_t>(m_sBox[(word >> 8) & 0xff]) << 8) |
             (static_cast<uint32_t>(m_sBox[(word >> 16) & 0xff]) << 16) |
             (static_cast<uint32_t>(m_sBox[word >> 24]) << 24);
    };
  ...
  bool cipherCTR(const void *in, size_t inLength, void *out,
    ...
    // Keep the first 64 bits fixed and increment the final 64 bits as a
    // big-endian integer, matching all hardware backends.
