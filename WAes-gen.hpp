...
protected:
  ...
  // Read 8 bytes from ptr as a big-endian uint64_t (portable, optimises to
  // bswap on LE)
...
private:
  ...
  uint8_t *invCipher(uint8_t *state) const {
    ...
    for (uint8_t r = 1; r < Nr; ++r, dk += 4) {
      // InvShiftRows: row1 from col(j+3)%4, row2 from col(j+2)%4, row3 from
      // col(j+1)%4
