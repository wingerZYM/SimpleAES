...
protected:
	...
	static const uint8_t m_gfmul_2[256];
	static const uint8_t m_gfmul_3[256];
	static const uint8_t m_gfmul_9[256];
	static const uint8_t m_gfmul_b[256];
	static const uint8_t m_gfmul_d[256];
	static const uint8_t m_gfmul_e[256];
};
...
const uint8_t aesTable::m_gfmul_2[256] = {
...
const uint8_t aesTable::m_gfmul_3[256] = {
...
const uint8_t aesTable::m_gfmul_9[256] = {
...
const uint8_t aesTable::m_gfmul_b[256] = {
...
const uint8_t aesTable::m_gfmul_d[256] = {
...
const uint8_t aesTable::m_gfmul_e[256] = {
...
private:
	...
	void mixColumns(uint8_t *state) const
	{
		...
		for (int i = 0; i < 4; ++i, state += 4)
		{
			...
			arr[2] = state[2];
			arr[3] = state[3];
			state[0] = m_gfmul_2[arr[0]] ^ m_gfmul_3[arr[1]] ^ arr[2] ^ arr[3];
			state[1] = arr[0] ^ m_gfmul_2[arr[1]] ^ m_gfmul_3[arr[2]] ^ arr[3];
			state[2] = arr[0] ^ arr[1] ^ m_gfmul_2[arr[2]] ^ m_gfmul_3[arr[3]];
			state[3] = m_gfmul_3[arr[0]] ^ arr[1] ^ arr[2] ^ m_gfmul_2[arr[3]];
		}
	}
	...
	void invMixColumns(uint8_t *state) const
	{
		...
		for (uint8_t i = 0; i < 4; ++i, state += 4)
		{
			...
			arr[2] = state[2];
			arr[3] = state[3];
			state[0] = m_gfmul_e[arr[0]] ^ m_gfmul_b[arr[1]] ^ m_gfmul_d[arr[2]] ^ m_gfmul_9[arr[3]];
			state[1] = m_gfmul_9[arr[0]] ^ m_gfmul_e[arr[1]] ^ m_gfmul_b[arr[2]] ^ m_gfmul_d[arr[3]];
			state[2] = m_gfmul_d[arr[0]] ^ m_gfmul_9[arr[1]] ^ m_gfmul_e[arr[2]] ^ m_gfmul_b[arr[3]];
			state[3] = m_gfmul_b[arr[0]] ^ m_gfmul_d[arr[1]] ^ m_gfmul_9[arr[2]] ^ m_gfmul_e[arr[3]];
		}
	}
