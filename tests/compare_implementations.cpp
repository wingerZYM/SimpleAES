 ...
 * Compares test results from different AES implementations to verify
 * consistency.
 *
 * This is a C++ port of the Python comparison tool to avoid Python
 * dependencies.
 */
...
struct TestRecord {
  ...
  std::string get_test_id() const {
    return mode + "_" + std::to_string(key_size) + "_" + padding + "_" +
...
public:
  ...
  std::vector<TestRecord> parse_test_file(const std::string &filename) {
    ...
    // Walk each heading and slice until the next heading (or EOF). Splitting
    // on the delimiter and dropping empty tokens used to lose the final test.
    std::regex test_section_regex(R"(=== Test: (.+?) ===)");
    const std::sregex_iterator end;
    for (std::sregex_iterator current(content.begin(), content.end(),
                                      test_section_regex);
         current != end; ++current) {
      const auto next = std::next(current);
      const size_t sectionStart =
          static_cast<size_t>(current->position() + current->length());
      const size_t sectionEnd =
          next == end ? content.size() : static_cast<size_t>(next->position());
      const std::string section =
          content.substr(sectionStart, sectionEnd - sectionStart);

      TestRecord record = parse_test_section((*current)[1].str(), section);
  ...
  TestRecord parse_test_section(const std::string &test_name,
                                const std::string &content) {
    TestRecord record;
    record.test_name = test_name;

    // Define regex patterns for extracting data
    std::regex patterns[] = {std::regex(R"(Implementation:\s*(.+))"),
                             std::regex(R"(Mode:\s*(.+))"),
  ...
  compare_implementations(const std::vector<std::string> &impls) {
    ...
    for (const auto &test_group : test_groups) {
      ...
      bool cipher_match =
          std::all_of(cipher_hashes.begin(), cipher_hashes.end(),
      ...
      for (const auto &record_pair : test_records) {
        if (record_pair.second.decrypted_hash !=
            record_pair.second.input_hash) {
      ...
      if (cipher_match && decrypt_integrity) {
        ...
        if (!cipher_match) {
          ...
          failure_detail +=
              "  Issue: Cipher output differs between implementations\n";
          for (const auto &record_pair : test_records) {
        ...
        if (!decrypt_integrity) {
          ...
          failure_detail +=
              "  Issue: Decrypted data doesn't match original input\n";
          for (const auto &record_pair : test_records) {
  ...
  bool run_implementation_test(const std::string &impl_name) {
    ...
    std::map<std::string, std::string> cmd_map = {
        {"Generic", "run-generic"}, {"AES-NI", "run-aes-ni"},
        {"VAES", "run-vaes"},       {"VAES512", "run-vaes512"},
        {"ARMv8", "run-armv8"},     {"WAes", "run-waes"}};
  ...
  bool
  check_cpu_feature_support(const std::vector<std::string> &feature_macros) {
  ...
  std::vector<std::string> check_implementation_availability() {
    ...
    if (is_x86_64) {
      // AES-NI implementation also uses SSSE3 shuffles.
      if (file_exists("../WAes-ni.hpp") &&
          check_cpu_feature_support({"__SSSE3__", "__AES__"})) {
        available.push_back("AES-NI");
      }

      // VAES: requires the AES-NI base plus AVX2 and VAES.
      if (file_exists("../WAes-vaes.hpp") &&
          check_cpu_feature_support(
              {"__SSSE3__", "__AES__", "__AVX2__", "__VAES__"})) {
        available.push_back("VAES");
      }

      // Check every AVX-512 extension used by the implementation.
      if (file_exists("../WAes-vaes512.hpp") &&
          check_cpu_feature_support({"__SSSE3__", "__AES__", "__AVX2__",
                                     "__VAES__", "__AVX512F__", "__AVX512BW__",
                                     "__AVX512DQ__", "__AVX512VL__"})) {
  ...
  int run_comparison(const std::vector<std::string> &target_impls) {
    ...
    if (!target_impls.empty()) {
      ...
      if (verbose) {
        std::cout << "Detecting available implementations with hardware "
                     "support check...\n";
      }
    ...
    if (failed == 0) {
      ...
      std::cout << "❌ " << failed
                << " tests FAILED! Implementations differ.\n";
      return 1;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (clean_only) {
    ...
    std::vector<std::string> all_possible = {"Generic", "AES-NI", "VAES",
                                             "VAES512", "ARMv8",  "WAes"};
